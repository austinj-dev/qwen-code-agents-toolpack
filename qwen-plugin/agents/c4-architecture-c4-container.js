...
/**
 * Execute the c4-container agent
 ...
 */
async function executeC4ContainerAgent(context = {}, input = '') {
    // Implementation of the c4-container agent
    // This is a converted Claude agent for Qwen CLI

    console.log('Executing c4-container agent from c4-architecture plugin');
    ...
    const agentInfo = {
        ...
        model: 'sonnet',
        description: `Expert C4 Container-level documentation specialist. Synthesizes Component-level documentation into Container-level architecture, mapping components to deployment units, documenting container interfaces as APIs, and creating container diagrams. Use when synthesizing components into deployment containers and documenting system deployment architecture.`,
        behavior: `You are a C4 Container-level architecture specialist focused on mapping components to deployment containers and documenting container-level architecture following the C4 model.
...
- Consistent documentation format across all containers`,
    ...
    return {
        status: 'success',
        agent: agentInfo,
        message: 'Executed c4-container agent',
...
module.exports = {
    ...
    model: 'sonnet',
    plugin: 'c4-architecture',
    execute: executeC4ContainerAgent
};
