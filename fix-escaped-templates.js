...
// Function to fix double-escaped GitHub Actions templates
function fixDoubleEscapedGitHubActions(content) {
  // Replace double-escaped GitHub Actions templates (\\${{) with single-escaped (\${{)
  content = content.replace(/\\\$\{\{/g, '\\${{');
  // Also fix double-escaped regular templates (\\${) with single-escaped (\${)  
  content = content.replace(/\\\$\{/g, '\\${');
  return content;
}
...
function processFiles() {
  ...
  for (const file of files) {
    try {
      ...
      const hasDoubleEscaped = /\\\$\{\{/.test(content) || /\\\$\{/.test(content);
      
      if (hasDoubleEscaped) {
        const originalContent = content;
        content = fixDoubleEscapedGitHubActions(content);
